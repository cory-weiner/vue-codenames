...
export default new Vuex.Store({
  state: {
    ...
    retain: true,
    active_team: '',
    player_type: 'spymaster',
    username: '',
    colors: { 'RED': '#DC143C', 'BLUE': '#6495ED', 'NUETRAL': '#FAEACB', 'ASSASSIN': '#333333' },
  ...
  getters: {
    getField,
    scoreboard(state) {
      if (!state.gameboard.tiles) {
        return false
      }
      let scoreboard = {}
      scoreboard['redcount'] = state.gameboard.tiles.filter(t => { return t.value == 'RED' }).length
      scoreboard['bluecount'] = state.gameboard.tiles.filter(t => { return t.value == 'BLUE' }).length
      scoreboard['redscore'] = state.gameboard.tiles.filter(t => { return t.value == 'RED' && t.guessed == true }).length
      scoreboard['bluescore'] = state.gameboard.tiles.filter(t => { return t.value == 'BLUE' && t.guessed == true }).length
      scoreboard['assassin_triggered'] = state.gameboard.tiles.filter(t => { return t.value == 'ASSASSIN' && t.guessed == true }).length
      scoreboard['win_condition_met'] = (scoreboard['redcount'] == scoreboard['redscore'] || scoreboard['bluecount'] == scoreboard['bluescore'] || scoreboard['assassin_triggered'])
      return scoreboard
    }
  },
  mutations: {
    ...
    publishPlayerList(state){
      state.client.publish(state.topic + state.lobby_id + '/players', JSON.stringify(state.players), { qos: parseInt(state.publish_qos), retain: state.retain })
    },
    addPlayer(state){

      state.players.push(state.username);
      this.commit("publishPlayerList")
    },
    guess(state, index) {
